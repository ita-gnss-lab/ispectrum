    ...
    do {
        ...
        do {
            ...
            do {
				...
				//printf("i: %22.15e\n",*i);
                *i += fp + fm;
				//printf("fm, fp, i: %22.15e %22.15e %22.15e\n",fm,fp,*i);
                if (m == 1) *err += fabs(fp) + fabs(fm);
                ep *= ehp;
                em *= ehm;
                tk += 1;
				//printf("i, ep, epsln: %22.15e %22.15e %22.15e\n",*i,ep,epsln);
				//exit(0);
            } while (ep < epsln);
            if (m == 1) {
                errh = *err * epsh;
        ...
        h *= 0.5;
        m *= 2;
		//printf(",i,errd,errh,m: %22.15e %22.15e %22.15e %d\n",*i,errd,errh,m);
    } while (errd > errh && m < mmax);
    *i *= h;
    if (errd > errh) {
        *err = -errd;
